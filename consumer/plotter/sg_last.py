...
df1 = pd.read_csv('../sg_sensor_alerts_final.csv')

# Convert 'TIMESTAMP' to datetime format in the first CSV
df1['TIMESTAMP'] = pd.to_datetime(df1['TIMESTAMP'], errors='coerce')

# Ensure 'SG_DATA' is numeric and drop NaN values
df1['SG_DATA'] = pd.to_numeric(df1['SG_DATA'], errors='coerce')
df1 = df1.dropna(subset=['SG_SENSOR', 'SG_DATA'])

# Read the second CSV file for comparison
df2 = pd.read_csv('../sg_sensor_aligned_final.csv')
...
unique_sensors = df1['SG_SENSOR'].unique()
...
for sensor in unique_sensors:
    ...
    sensor_data1 = df1[df1['SG_SENSOR'] == sensor]
    ...
    sns.scatterplot(x='TIMESTAMP', y='SG_DATA', data=sensor_data1, color='gray', s=20)
    ...
    # Highlight alerts based on SG_STATUS with reduced dot size
    for status, color in alert_colors.items():
        alerts = sensor_data1[sensor_data1['SG_STATUS'] == status]
        plt.scatter(alerts['TIMESTAMP'], alerts['SG_DATA'], color=color, s=50, zorder=5, label=status)
    
    # Plot the sorted comparison data from the second CSV as a flat line
    plt.plot(sensor_data2['TIMESTAMP'], sensor_data2['DATA'], color='purple', linestyle='-', linewidth=1.5, label='Comparison Data')
    
    # Add labels and title
    plt.xlabel('Timestamp')
    plt.ylabel('SG_DATA')
    plt.title(f'Time Series of SG_DATA with Alerts, Thresholds, and Comparison for {sensor} Sensor')
    plt.legend()
